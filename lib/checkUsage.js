...
module.exports = function(program){
  ...
  if(!err){
    if(process.env.ELASTICSEARCH_PORT){
      messages.push('Elasticsearch Docker container found at: '+ process.env.ELASTICSEARCH_PORT+'. Connecting...\n');
    }else{
    ...
    if(program.bucket && !program.profile){
      if(process.env.AWS_ACCESS_KEY_ID){
        messages.push('Using AWS environment variables for credentials.\n');
      }else{
