...
module.exports = function(program, env){
  ...
  if(!program.data&&!program.bucket){
    messages.push('Must provide data as a shapefile, geodatabase, GeoJson, csv, gz, or zip.\nYou can also provide an S3 bucket.\n');
    err = 1;
  }
  ...
  if((path.extname(program.data) === '.csv'
  || path.extname(path.basename(program.data, '.gz')) === '.csv')
  && !program.preformatted
  ){
    messages.push('Only accepts preformatted .csv data. Run through ogr2ogr specifying your transform and passing -lco GEOMETRY=AS_XY to get spatial data in a form accessible by the loader. Then specify the --preformatted flag to the loader.');
    err = 1;
  }
  ...
  if(!err){
    ...
    messages.push('Loading data into ' + program.alias + '/' + program.type + '.\n');
