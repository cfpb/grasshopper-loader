  ...
  function processRequest(stream, record){
    ...
    checkHash(stream, record.hash, function(hashIsEqual, remoteHash){
      if(hashIsEqual){
        ...
        if(options.monitor || record._retrieverProcessed) return recordCallback(null, record);
        return;
      }
    ...
    if(zipReg.test(record.url)){
      handleZip(record, stream, scratchSpace, handleStream, handleStreamError);
    }else{
      if(csvReg.test(record.file)){
        handleCsv(record, stream, scratchSpace, handleStream, handleStreamError);
      }else{
